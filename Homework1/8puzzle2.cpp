...
pair<bool, int> search(const Node& node, int threshold, int currentCost, queue<tuple<Node, int, int>>& nodesToSkipTo) {
    ...
    if (currentCost + node.getDistance() > threshold) {
        nodesToSkipTo.push(tuple(node, currentCost + node.getDistance(), currentCost));
    ...
    if ((zeroCoordinates.y >= 0 && zeroCoordinates.y < dimension) && 
        ...
        // cout << "in left" << endl;
        pair<bool, int> leftRes = search(Node(node, Move::left), threshold, currentCost + 1, nodesToSkipTo);
    ...
    if ((zeroCoordinates.y > 0 && zeroCoordinates.y <= dimension) &&
        ((!node.getMoves().empty() && node.getMoves().back() != Move::left) || node.getMoves().empty())) {
            // cout << "in right" << endl;
        pair<bool, int> rightRes = search(Node(node, Move::right), threshold, currentCost + 1, nodesToSkipTo);
    ...
    if ((zeroCoordinates.x > 0 && zeroCoordinates.x <= dimension) &&
        ...
        // cout << "in down" << endl;
        pair<bool, int> downRes = search(Node(node, Move::down), threshold, currentCost + 1, nodesToSkipTo);
    ...
    if ((zeroCoordinates.x >= 0 && zeroCoordinates.x < dimension) &&
        ...
        // cout << "in up" << endl;
        pair<bool, int> upRes = search(Node(node, Move::up), threshold, currentCost + 1, nodesToSkipTo);
...
void IDAStar(const Node& start) {
    ...
    queue<tuple<Node, int, int>> nodesToSkipTo;
    nodesToSkipTo.push(tuple(start, threshold, 0));
    int whileiterations = 1;
    while(!nodesToSkipTo.empty()) {
        cout << "iter: " << whileiterations++ << endl;
        pair<bool, int> iterationResult = search(get<0>(nodesToSkipTo.front()), get<1>(nodesToSkipTo.front()), get<2>(nodesToSkipTo.front()), nodesToSkipTo);
        nodesToSkipTo.pop();
